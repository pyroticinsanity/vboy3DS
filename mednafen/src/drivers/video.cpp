...
#ifdef WII
      //
      // Use the back surface we created
      //
      screen = back_surface;      
#else
...
#if BPP == 8
  source_surface->pixels = msurface->pixels8;
  source_surface->pitch = msurface->pitch32 * sizeof(uint8);
#elif BPP == 16
  source_surface->pixels = msurface->pixels16;
  source_surface->pitch = msurface->pitch32 * sizeof(uint16);
#else 
